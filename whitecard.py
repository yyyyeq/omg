...
# ---------------------------------------------------------
st.set_page_config(page_title="💳 白卡團建費登記系統", layout="wide", page_icon="💳")
...
def get_expenses():
    try:
        response = supabase.schema("public").table("whitecard_expenses").select("*").execute()
...
with st.form("add_expense_form", clear_on_submit=True):
    ...
    if submitted:
        ...
        if not exp_snack.strip():
            ...
            supabase.schema("public").table("whitecard_expenses").insert(new_data).execute()
...
def edit_record_dialog(row_id, row_data):
    ...
    if st.button("儲存修改", type="primary", use_container_width=True):
        ...
        supabase.schema("public").table("whitecard_expenses").update(updated_data).eq("id", row_id).execute()
        st.success("修改成功！")
        st.rerun()
    ...
    for _, row in expenses_df.iterrows():
        ...
        with card_col7:
            if st.button("🗑️ 刪除", key=f"del_{row['id']}", use_container_width=True):
                supabase.schema("public").table("whitecard_expenses").delete().eq("id", row['id']).execute()
                st.rerun()
